...
class Data:
    ...
    def fill_na_train(self, df: pd.DataFrame) -> pd.DataFrame:
        for price in PRICES: 
            svr = SVR(kernel="rbf", gamma=self.gamma, C=self.C)
            missing_price = df[price].isnull()

            # Fit the model on the cuttent price
            df_model = df.loc[~missing_price, [price, "price_CHF"]].copy()
            X = df_model["price_CHF"].copy().T.values.reshape(-1, 1)
            ...
            df_missing = df.loc[missing_price, [price, "price_CHF"]]
            predicted_values = svr.predict(df_missing["price_CHF"].T.values.reshape(-1, 1))
